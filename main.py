...
def get_user_data(user_id):
  ...
  c = conn.cursor()
  c.execute(
      'SELECT coins, last_omikuji FROM users WHERE user_id = ?', (user_id,)
  )
  result = c.fetchone()
  conn.close()
  if result:
...
def add_monster(user_id, monster_name):
  ...
  c = conn.cursor()
  c.execute(
      'INSERT INTO inventory (user_id, monster_name) VALUES (?, ?)',
...
def set_target_id(user_id):
  ...
  c = conn.cursor()
  c.execute(
      'INSERT OR REPLACE INTO aoru_target (id, target_user_id) VALUES (1, ?)',
      (user_id,),
  )
  conn.commit()
  conn.close()
...
async def get_or_update_lucky_member(guild):
  ...
  c.execute(
      'INSERT OR REPLACE INTO lucky_member (id, user_id, lucky_date) VALUES'
      ' (1, ?, ?)',
...
class MyBot(commands.Bot):
  ...
  async def setup_hook(self):
    init_db()
    print(
        f'✅ おみくじ＆ガチャ＆煽り＆ラッキーメンバーBot 起動準備完了'
    )
...
# --- コマンド同期用プレフィックスコマンド (管理者限定: !sync) ---
@bot.command(name='sync')
@commands.has_permissions(administrator=True)
async def sync(ctx):
  """スラッシュコマンドを追加・更新したときだけ `!sync` で手動同期する"""
...
# --- 毒舌おみくじコマンド ---
@bot.tree.command(
    name='omikuji', description='毒舌おみくじを引いてガチャコインをゲット！'
)
async def omikuji(interaction: discord.Interaction):
  ...
  FORTUNES = {
      ...
      '超大吉': {
          'i': '🎆',
          'c': 0xFFD700,
          'm': '今日のお前はまあまあ運気があるじゃないか！',
      },
      '大吉': {
          'i': '🌟',
          'c': 0xFFD700,
          'm': 'ヘッツ！大吉かよ！まあ運はあるんじゃないか？',
      },
      '中吉': {
          'i': '✨',
          'c': 0x32CD32,
      ...
      '凶': {
          'i': '🪦',
          'c': 0x4B0082,
          'm': 'おいおい！凶かよ！どんだけ運が悪いんだｗ',
      },
      '大凶': {
          'i': '👻',
          'c': 0x000000,
          'm': '大凶とかｗ 今日は外に出ないほうがいいんじゃねーか？',
      },
      '首の皮一枚': {
          'i': '🩻',
          'c': 0x696969,
          'm': (
              '首の皮一枚でつながった運勢か．．．お前大丈夫か？'
          ),
      },
  }
  ...
  embed = discord.Embed(
      title=f'⛩️ {interaction.user.display_name}さんの運勢', color=data['c']
  )
  embed.add_field(
      name=f"{data['i']} {key}", value=f"**{data['m']}**", inline=False
  )
  embed.add_field(
      name='🎁 特典',
      value='**ガチャコインを3枚** 手に入れました！',
      inline=False,
  )
  embed.set_footer(
      text=(
          f'現在の所持コイン: {new_coins}枚 |'
          ' 明日もまた引かせてやるよ'
      )
  )
...
# --- ガチャコマンド ---
@bot.tree.command(
    name='gacha', description='コインを1枚使ってモンスターを召喚！'
)
async def gacha(interaction: discord.Interaction):
  ...
  MONSTER_DATA = {
      '✨ 伝説のたいが神': (
          'https://raw.githubusercontent.com/2401147/-werewolf-bot3/main/a.png'
      ),
      '👑 島さんの弟': (
          'https://raw.githubusercontent.com/2401147/-werewolf-bot3/main/d.png'
      ),
      '🔥 ゆずの皮': (
          'https://raw.githubusercontent.com/2401147/-werewolf-bot3/main/c.png'
      ),
      '⚡ みかんの皮': (
          'https://raw.githubusercontent.com/2401147/-werewolf-bot3/main/b.png'
      ),
      '🐼 パンダ顔のおっさん': (
          'https://raw.githubusercontent.com/2401147/-werewolf-bot3/main/e.png'
      ),
      '🐈 猫舌男': (
          'https://raw.githubusercontent.com/2401147/-werewolf-bot3/main/h.png'
      ),
      '💧 ニート': (
          'https://raw.githubusercontent.com/2401147/-werewolf-bot3/main/f.png'
      ),
      '🦾 ただのおっさん': (
          'https://raw.githubusercontent.com/2401147/-werewolf-bot3/main/g.png'
      ),
  }
  ...
  embed.add_field(
      name='召喚結果', value=f'**{monster_name}** ({rarity})', inline=False
  )
...
# --- コレクション確認コマンド ---
@bot.tree.command(
    name='collection', description='仲間にしたモンスターを確認する'
)
async def collection(interaction: discord.Interaction):
  ...
  if not monsters:
    await interaction.response.send_message(
        'まだモンスターを1匹も持ってないな。寂しい奴め！', ephemeral=True
    )
    return
  ...
  await interaction.response.send_message(
      f'👾 **{interaction.user.display_name}のコレクション**\n{msg}'
  )


# --- ターゲット設定コマンド (管理者限定) ---
@bot.tree.command(
    name='set_target', description='【運営専用】煽りターゲットを設定する'
)
...
AORU_MESSAGES = [
    ...
    (
        'ちょっと <@{user_id}>'
        ' さん、またくだらないこと言ってますね～ｗ'
    ),
    (
        '<@{user_id}>'
        ' が何か言いたそうにこちらを見ている！…が、誰も気にしていない！'
    ),
...
@bot.tree.command(
    name='aoru', description='設定されたターゲットをみんなで煽る！'
)
async def aoru(interaction: discord.Interaction):
  ...
  if not target_id:
    await interaction.response.send_message(
        'まだターゲットが設定されてねーぞ！運営に `/set_target` させろ！',
        ephemeral=True,
    )
    return
...
LUCKY_COMMENTS = [
    ...
    (
        '🎉 本日のMVP（ラッキー）は <@{user_id}>！'
        ' ジュース奢ってもらえよ！'
    ),
    (
        '👑 本日のラッキーメンバーは <@{user_id}> だ！'
        ' 調子に乗るなよｗ'
    ),
]
...
@bot.tree.command(
    name='lucky', description='今日のラッキーメンバーを確認する！'
)
...
async def on_message(message: discord.Message):
  ...
  # ターゲット本人が発言した時、0.3% (0.003) の確率で自動で煽りレスをする
